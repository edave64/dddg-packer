...
import type { HeadDummy } from "@/store/head-dummy";
...
import { useActivePack } from "./active-pack";

export type V1Pack = V1.JSONCharacter<HeadDummy>;
export type V2Pack = V2.JSONContentPack;

export const dependencies = computed(() => {
	return (useActivePack().value?.dependencies as string[]) ?? [];
});

const normalizedDependecyTree = computedAsync<INormalizedPack>(
	async () => {
...
export function useNormalizedDependecyTree() {
	return normalizedDependecyTree;
}
